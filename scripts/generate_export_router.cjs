...
exportsRouter.post("/requests", requireAuth, requireRoles(RESEARCH_ROLES), async (c) => {
  ...
  await db.prepare(\`
    INSERT INTO dataset_export_requests (
      id, requester_user_id, role, request_type, dataset_type, scope_level,
      course_id, cohort_id, student_id, requested_anonymization_level,
      status, purpose, justification, field_filters_json
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?, ?)
  \`).bind(
...
exportsRouter.post("/requests/:id/approve", requireAuth, requireRoles(["admin"]), async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
  ...
  await db.prepare(\`
    ...
    SET status = 'approved', approved_anonymization_level = ?, approved_by = ?, approved_at = CURRENT_TIMESTAMP, max_download_count = ?
    WHERE id = ?
  \`).bind(
...
exportsRouter.post("/requests/:id/reject", requireAuth, requireRoles(["admin"]), async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
  const id = c.req.param("id");
  const body = await c.req.json();
...
exportsRouter.post("/requests/:id/generate", requireAuth, requireRoles(["admin", "researcher", "collaborator"]), async (c) => {
  const db = c.env.DB;
  const id = c.req.param("id");
  const user = c.get("user");
  ...
  await db.prepare(\`
    UPDATE dataset_export_requests
    SET status = 'generated', export_file_path = ?, row_count = ?, summary_json = ?
    WHERE id = ?
  ...
  return c.json({ success: true, id, status: "generated" });
});
...
console.log('exports.ts generated');

const dataTsFile = '/home/user/webapp/src/api/routes/data.ts';
let dataTsContent = fs.readFileSync(dataTsFile, 'utf8');

if (!dataTsContent.includes('exportsRouter')) {
  dataTsContent = dataTsContent.replace(
    /import \{ Hono \} from "hono";/,
    'import { Hono } from "hono";\nimport exportsRouter from "./exports";'
  );
  
  // mount the router inside dataRouter
  dataTsContent += '\n\ndataRouter.route("/exports", exportsRouter);\n';
  fs.writeFileSync(dataTsFile, dataTsContent);
  console.log('exportsRouter mounted in data.ts');
}
