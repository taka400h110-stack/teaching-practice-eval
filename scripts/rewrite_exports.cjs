...
// I will just replace the whole file since it's easier to maintain the exact logic.
const newContent = `
...
exportsRouter.post("/requests/:id/generate", requireAuth, requireRoles(["admin", "researcher", "collaborator"] as UserRole[]), async (c) => {
  ...
  if (c.env.EXPORTS_BUCKET) {
    await c.env.EXPORTS_BUCKET.put(objectKey, dataString, {
      httpMetadata: {
        ...
        contentDisposition: \`attachment; filename="\${datasetType}-\${anonLevel}.json"\`,
  ...
  await db.prepare(\`
    ...
    WHERE id = ?
  \`).bind(
    objectKey, rowCount, JSON.stringify({ dataset: datasetType, anonLevel }), 
    objectKey, byteSize, hash, id
  ).run();
...
fs.writeFileSync('/home/user/webapp/src/api/routes/exports.ts', newContent);
console.log("Updated exports.ts");
