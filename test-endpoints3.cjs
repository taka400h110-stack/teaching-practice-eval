...
async function runTests() {
  ...
  console.log("=== Testing with Researcher ===");
  const resLoginRes = await request('POST', '/api/data/auth/login', {
    email: 'researcher@teaching-eval.jp',
    password: 'password'
  });
  console.log(`Login Status: ${resLoginRes.status}`);
  const resToken = resLoginRes.data.token;

  const resCleanupRes = await request('GET', '/api/admin/alerts/cleanup-failure', null, resToken);
  console.log(`GET /api/admin/alerts/cleanup-failure - Status: ${resCleanupRes.status}`);
  if (resCleanupRes.status !== 200) console.log(resCleanupRes.data);
