...
// 2. replace handleLGCM logic to do real calculation
content = content.replace(/const handleLGCM = useCallback\(async \(\) => \{[\s\S]*?setLgcmDone\(true\);\n  \}, \[cohorts\]\);/, 
...
// 4. replace LGCM_RESULT with lgcmResult (in JSX only)
// But LGCM_RESULT is already redefined as an empty obj so it shouldn't crash, but it's better to use lgcmResult in JSX
// let's do a simple regex for `{LGCM_RESULT.` -> `{lgcmResult?.`
content = content.replace(/\{LGCM_RESULT\./g, '{lgcmResult?.');

// Fix the Array map keys in LCGA Plot since LCGA_CLASSES mapped 'id' to String
content = content.replace(/row\[cls\.id\] = cls\.trajectory\[i\]\.score;/g, ''); // already done but just in case
...
// Also downloadLGCMCSV has LGCM_RESULT
content = content.replace(/LGCM_RESULT\./g, 'lgcmResult?.');
