...
const RUBRIC_FACTORS = [
  {
    key: "F1", label: "多様性・共生理解", color: "#1976d2",
    items: [
  ...
  {
    key: "F2", label: "授業設計・実施", color: "#388e3c",
    items: [
      { num: 8,  label: "教科の目標・内容理解" },
      { num: 9,  label: "学習目標の設定" },
      { num: 10, label: "授業構成・展開の工夫" },
      { num: 11, label: "教材の選択・活用" },
      { num: 12, label: "ICT・多様な指導法" },
      { num: 13, label: "評価方法の設定" },
    ],
  },
  {
    key: "F3", label: "学習支援・学級経営", color: "#f57c00",
    items: [
      { num: 14, label: "個別の学習支援" },
      { num: 15, label: "学習意欲の促進" },
      { num: 16, label: "学級の雰囲気づくり" },
      { num: 17, label: "保護者・地域連携" },
    ],
  },
  {
    key: "F4", label: "省察・専門的成長", color: "#7b1fa2",
    items: [
      { num: 18, label: "授業後の自己評価" },
      { num: 19, label: "省察の深さ（RD）" },
      { num: 20, label: "フィードバックの受容" },
      { num: 21, label: "改善策の立案・実行" },
      { num: 22, label: "専門的知識の更新" },
      { num: 23, label: "倫理的・責任ある行動" },
    ],
  },
];
...
function generateCoTResponse(userMsg: string, rdLevel: number): string {
  ...
  if (lower.includes("授業") || lower.includes("教材") || lower.includes("学習目標") || lower.includes("指導")) {
    if (rdLevel <= 2) {
      return `ルーブリック F2（授業設計・実施）に関連しますね。\n\n設定した学習目標は児童の実態に合っていましたか？使った教材や指導法の効果を、児童の反応から具体的に教えてください。[RD-${rdLevel}]`;
    }
    return `F2の観点から深い省察ができています。\n\n授業設計の改善点を SMART目標として具体化してみましょう。例：「第〇週までに〇〇を実施し、評価項目8-13で平均〇点以上を目指す」[RD-${rdLevel}]`;
  }

  // F3: 学習支援・学級経営
  if (lower.includes("学級") || lower.includes("児童") || lower.includes("生徒") || lower.includes("クラス")) {
    if (rdLevel <= 2) {
      return `ルーブリック F3（学習支援・学級経営）の視点ですね。\n\n学級の雰囲気や児童間の関係性について、どのような変化を感じましたか？あなたの働きかけとどんな関係がありそうですか？[RD-${rdLevel}]`;
    }
    return `F3の深い省察ですね。学級経営の改善に向けた具体的なアクションプランを立ててみましょう。[RD-${rdLevel}]`;
  }

  // F4: 省察・専門的成長
  if (lower.includes("省察") || lower.includes("振り返") || lower.includes("フィードバック") || lower.includes("成長")) {
    if (rdLevel <= 2) {
      return `ルーブリック F4（省察・専門的成長）に関連しています。\n\nその気づきは、以前の自分の考え方とどう違いますか？その変化はなぜ起きたと思いますか？[RD-${rdLevel} → より深い省察を目指しましょう]`;
    }
    if (rdLevel >= 4) {
  ...
  // デフォルト（省察深さに応じた深化促進）
  if (rdLevel <= 2) {
    return `ありがとうございます。\n\nその経験について「なぜそうなったのか」「どうすれば改善できるか」を、ルーブリックの4つの因子（多様性・授業設計・学習支援・省察）の観点から振り返ってみましょう。[現在の省察深さ: RD-${rdLevel}]`;
  }
...
export default function ChatBotPage() {
  ...
  const PHASE_PROMPTS: Record<string, string[]> = {
    ...
    phase1:    [
      "今日の授業でF1（多様性への対応）で気づいたことは？",
      "授業設計（F2）で改善できた点・できなかった点を教えてください",
      "学習支援（F3）で児童の反応はどうでしたか？",
      "フィードバックを受けてどう感じましたか？（F4）",
    ],
